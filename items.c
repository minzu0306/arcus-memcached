                                      ...
                                      btree_indx_node *p_node, btree_indx_node *c_node, int indx)
{
    ...
    if (c_node->next != NULL)  c_node->next = NULL; c_node->next->prev = NULL;
    p_node->used_count--;
...
{
    btree_indx_node *node;
//printf("info->root->ndepth = %d\n", info->root->ndepth);
//printf("info->root->used_count = %d\n", info->root->used_count);
    if (info->root->ndepth == 0) {
//printf("root node unlink\n");
        assert(info->root->used_count == 0);
        do_btree_node_free(engine, info->root);
        info->root = NULL;
//printf("root node unlink finish\n");
    } else {
...
printf("do_btree_node_remove success\n");
}
